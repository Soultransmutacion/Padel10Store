...
/**
 * URL del webhook de confirmacion de pagos. Todavia no existe ningun
 * endpoint que reciba estas notificaciones (queda fuera de alcance de
 * esta etapa): se prepara el campo notification_url porque Mercado Pago
 * lo acepta desde ahora y asi no hace falta recrear la preferencia mas
 * adelante. Mientras el endpoint no exista, Mercado Pago simplemente no
 * podra notificar; el estado del pedido no depende del webhook todavia.
 */
function buildNotificationUrl(trustedBaseUrl) {
  if (!trustedBaseUrl || typeof trustedBaseUrl !== 'string') return null;
  return `${trustedBaseUrl}/api/webhook-mercadopago`;
}
