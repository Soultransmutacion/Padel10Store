...
async function crearPedido(input, client) {
  ...
  // esta funcion (propiedad payment_retry_token, ver mas abajo).
  ...
  // payment_retry_token en claro: se adjunta como propiedad NO persistida
  // (la fila real en `data` solo tiene payment_retry_token_hash). Es
  // responsabilidad del llamador (api/pedidos.js) decidir si hace falta
  // devolverselo al cliente, y nunca debe loguearse ni reenviarse tal
  // cual a ningun otro sistema (ver lib/payment-retry-token.js).
  return Object.assign({}, data, { payment_retry_token: paymentRetryToken });
}
