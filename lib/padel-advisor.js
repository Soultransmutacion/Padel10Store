...
function buildOutputForModel(output) {
  ...
  // recomendar_productos (Fase 2 - Etapa 5): scoreInterno es unicamente para
  // ordenar, explicar internamente y testear (ver lib/padel-recommender.js).
  // El pedido original es explicito: "el score interno NO debe mostrarse al
  // usuario todavia" y "NO quiero porcentajes de compatibilidad". Se saca
  // aca, antes de que el numero llegue al modelo, para que sea imposible que
  // el modelo lo repita, lo convierta en un porcentaje o lo use como una
  // estrella/puntaje visible: el modelo solo ve productId, motivos y
  // advertencias reales, nunca el numero.
  if (output && output.ok && Array.isArray(output.resultados)) {
    const clone = Object.assign({}, output);
    clone.resultados = output.resultados.map((r) => {
      if (r == null || typeof r !== 'object') return r;
      const sinScore = Object.assign({}, r);
      delete sinScore.scoreInterno;
      return sinScore;
    });
    return clone;
  }
...
function collectCards(toolName, output, cardsById, context) {
  ...
  // recomendar_productos (Fase 2 - Etapa 5) devuelve un ranking, no una
  // busqueda por texto: cada resultado usa "productId" (no "id", ver
  // lib/padel-recommender.js) porque su shape es el explicable que pide el
  // pedido original (productId, scoreInterno, motivos, advertencias), no el
  // resumen de catalogo. Mismo criterio que buscar_catalogo/filtrar_palas:
  // todos los IDs rankeados quedan como resultados permitidos para una
  // eventual seleccion posterior por ver_producto, y si el ranking devuelve
  // un unico resultado, su tarjeta se adjunta directamente.
  if (toolName === 'recomendar_productos' && Array.isArray(output.resultados)) {
    output.resultados.forEach((resultado) => {
      if (resultado != null && typeof resultado.productId === 'string') {
        ctx.searchResultIds.add(resultado.productId);
      }
    });
    if (output.resultados.length === 1) {
      const unico = output.resultados[0];
      if (unico != null && typeof unico.productId === 'string' && cardsById.has(unico.productId) === false) {
        const card = verProducto(unico.productId);
        if (card) cardsById.set(card.id, card);
      }
    }
    return;
  ...
  // elegir_producto_para_usuario (Fase 2 - Etapa 6): igual que
  // recomendar_productos, todos los candidatos evaluados quedan como
  // resultados permitidos para una eventual seleccion posterior por
  // ver_producto. Si el codigo determino un ganador claro (hayGanador), su
  // tarjeta se adjunta directamente: es el unico producto puntual real que
  // esta respuesta esta recomendando "para el cliente", el mismo criterio
  // que ya usa el caso de un unico resultado en buscar_catalogo/filtrar_palas
  // /recomendar_productos. Si no hay ganador (empate o falta de informacion),
  // no se adjunta ninguna tarjeta nueva: no hay una eleccion real que
  // mostrar todavia.
  if (toolName === 'elegir_producto_para_usuario' && Array.isArray(output.resultados)) {
    output.resultados.forEach((resultado) => {
      if (resultado != null && typeof resultado.productId === 'string') {
        ctx.searchResultIds.add(resultado.productId);
      }
    });
    if (output.hayGanador && typeof output.ganador === 'string' && cardsById.has(output.ganador) === false) {
      const card = verProducto(output.ganador);
      if (card) cardsById.set(card.id, card);
    }
  }
}
...
function updateOfferedProducts(toolName, output, offeredContext) {
  ...
  // recomendar_productos (Fase 2 - Etapa 5): la lista de ofrecidos pasa a
  // ser el ranking devuelto por el recomendador, EN ESE ORDEN (el mejor
  // resultado primero), para que referencias conversacionales como "la
  // primera", "la mejor" o "la segunda" sigan resolviendo exactamente contra
  // el mismo orden que ya vio el cliente (mismo mecanismo que ya usan
  // buscar_catalogo/filtrar_palas/ver_producto/comparar_productos, nunca uno
  // nuevo).
  if (toolName === 'recomendar_productos' && Array.isArray(output.resultados)) {
    const ids = output.resultados.map((r) => r && r.productId).filter((id) => typeof id === 'string');
    if (ids.length > 0) offeredContext.ids = ids.slice(0, MAX_OFRECIDOS);
    return;
  }

  // elegir_producto_para_usuario (Fase 2 - Etapa 6): solo se reemplaza la
  // lista de ofrecidos cuando el codigo determino un ganador claro
  // (hayGanador), y en ese caso pasa a ser el UNICO producto ofrecido: es
  // "esa" para el turno siguiente (por ejemplo "Dale, agregame esa."), igual
  // que ya hace ver_producto con un resultado puntual. Si hay empate o falta
  // informacion, no se toca offeredContext: la lista de candidatos original
  // (comparados/ofrecidos) sigue siendo resolvible tal cual estaba.
  if (toolName === 'elegir_producto_para_usuario' && output.hayGanador && typeof output.ganador === 'string') {
    offeredContext.ids = [output.ganador];
  }
}
