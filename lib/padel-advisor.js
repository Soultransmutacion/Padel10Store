...
function collectCards(toolName, output, cardsById) {
  ...
  // Busqueda estructurada (buscar_catalogo o filtrar_palas): si el filtro
  // devolvio un unico producto exacto, se adjunta su tarjeta completa de
  // forma deterministica. Antes, la tarjeta solo se generaba si el modelo
  // volvia a llamar a ver_producto; si respondia directamente con los datos
  // del resumen (precio, talles) sin esa segunda llamada, no se enviaba
  // ninguna tarjeta al cliente aunque el producto estuviera identificado sin
  // ambiguedad (bug real: 'Busco una pollera negra de mujer, talle M.').
  if (
    (toolName === 'buscar_catalogo' || toolName === 'filtrar_palas') &&
    Array.isArray(output.resultados) &&
    output.resultados.length === 1
  ) {
    const unico = output.resultados[0];
    if (unico && typeof unico.id === 'string') {
