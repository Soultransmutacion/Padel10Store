...
const { normalizeText, verProducto } = require('./padel-catalog');
...
// de contacto que ya arma padel-advisor-tools.js. Las demas herramientas no
// exponen datos de WhatsApp, asi que se devuelven sin cambios.
...
function collectCards(toolName, output, cardsById) {
  ...
  // Busqueda estructurada (buscar_catalogo o filtrar_palas): se adjunta
  // la tarjeta completa de cada resultado devuelto por el filtro, de
  // forma deterministica. Antes, la tarjeta solo se generaba si el
  // modelo volvia a llamar a ver_producto, o si el filtro devolvia un
  // unico resultado; si el modelo respondia directamente con los datos
  // del resumen (precio, talles) sin una segunda llamada, o el filtro
  // devolvia varios candidatos, ninguna tarjeta llegaba al cliente
  // aunque el producto recomendado estuviera identificado sin
  // ambiguedad en el texto final (bug real: 'Busco una pollera negra
  // de mujer, talle M.'). La eleccion final de que tarjeta mostrar se
  // resuelve despues, en filterCardsByMention, comparando el texto de
  // la respuesta con cada candidato.
  ...
  if (esBusquedaEstructurada && Array.isArray(output.resultados)) {
    output.resultados.forEach((resultado) => {
      const yaExiste = resultado == null ? true : cardsById.has(resultado.id);
      if (resultado != null && typeof resultado.id === 'string' && yaExiste === false) {
        const card = verProducto(resultado.id);
