...
// Palabras genericas (verbos, articulos, preposiciones, saludos) que la gente
// usa al escribir pero que nunca forman parte del nombre, la marca, el tipo
// o la seccion de un producto real del catalogo. Se descartan antes de medir
// la cobertura de la busqueda determinista para no diluir la senal con
// palabras que no identifican ningun producto.
const QUERY_STOPWORDS = new Set([
  'busco', 'buscando', 'buscamos', 'una', 'un', 'unas', 'unos', 'de', 'del',
  'la', 'el', 'los', 'las', 'para', 'quiero', 'quisiera', 'necesito', 'tengo',
  'hola', 'buenas', 'buenos', 'dias', 'tardes', 'noches', 'por', 'favor',
  'porfa', 'porfavor', 'me', 'interesa', 'interesan', 'con', 'sin', 'y', 'o',
  'en', 'a', 'que', 'cual', 'cuales', 'hay', 'tienen', 'tenes', 'tienes',
  'tiene', 'talle', 'talla', 'como', 'al', 'mi', 'tu', 'se', 'es', 'esta',
  'este', 'esa', 'ese', 'gracias', 'consulta', 'consultar', 'ver', 'mostrar',
  'muestrame', 'recomendame', 'recomiendame', 'recomendar', 'algun', 'alguna',
  'algo', 'nada', 'todo', 'todos', 'todas', 'mas', 'chau',
]);

function extractSignalTokens(normalized) {
  return tokenize(normalized).filter(function (t) {
    return t.length > 1 && QUERY_STOPWORDS.has(t) === false;
  });
}

// Busqueda determinista de catalogo: se ejecuta siempre desde la logica de la
// aplicacion (nunca depende de que el modelo decida invocar buscar_catalogo).
// Extrae del mensaje crudo del usuario las palabras "senal" (descartando
// verbos, articulos, preposiciones y saludos que nunca aparecen en el
// nombre/marca/tipo/seccion de un producto) y exige cobertura total (100%)
// de esas palabras senal contra ese texto para considerar a un producto una
// coincidencia exacta. Si un unico producto alcanza esa cobertura total, es
// la unica fuente de verdad para adjuntar su tarjeta sin esperar una tool
// call. Si varios la alcanzan, la consulta es ambigua y no se elige ninguno
// arbitrariamente. Una consulta generica sin palabras senal (por ejemplo
// "hola") siempre devuelve una lista vacia.
function deterministicSearch(rawMessage) {
  const normalized = normalizeText(rawMessage);
  const signalTokens = extractSignalTokens(normalized);
  if (signalTokens.length === 0) return [];
  const catalog = loadCatalog();
  const exact = catalog.filter(function (p) {
    const haystackTokens = new Set(tokenize(buildHaystack(p)));
    return signalTokens.every(function (t) { return haystackTokens.has(t); });
  });
  return exact.map(toSummary);
}
...
function getValidatedTalles(product) {
  if (!Array.isArray(product.talles)) return null;
  const valid = product.talles.filter((t) => typeof t === 'string' && t.trim().length > 0);
...
function toSummary(product) {
  ...
  return {
    ...
    talles: getValidatedTalles(product),
  };
}
...
// Construye la tarjeta de comparacion visual (Fase 2 - Etapa 2) a partir de
// productos reales YA encontrados en el catalogo (nunca por texto libre ni
// por datos que hayan viajado desde el modelo o el navegador). No duplica
// logica: reutiliza exactamente las mismas fuentes de verdad que toCard/
// toComparisonEntry (isPrecioConsultar, formatPrice, resolveImageUrl,
// TECH_FIELD_KEYS/TECH_FIELD_LABELS) para que la tarjeta visual nunca pueda
// mostrar un precio, una imagen o un atributo distinto de los que ya ve el
// modelo o el resto del sitio.
//
// Reglas de datos (impuestas aca, no en la capa de herramientas):
// - Un atributo que ninguno de los productos tiene confirmado no genera fila
//   (nunca se muestra una fila vacia ni "No confirmado" en ambos lados).
// - Un atributo confirmado en al menos un producto genera una fila para
//   TODOS los productos comparados: el lado sin dato real se marca "No
//   confirmado" (nunca se inventa ni se copia el valor del otro producto).
// - El orden de las filas es siempre el de TECH_FIELD_KEYS (el mismo orden
//   estable ya usado por getConfirmedTechFields/toComparisonEntry), nunca el
//   orden en que aparecen los productos ni un orden decidido por el modelo.
// - El precio se expone una sola vez, dentro de "productos" (igual que en
//   toCard/toComparisonEntry): no se repite como fila para no duplicar el
//   mismo dato en dos formas distintas de la misma respuesta.
function buildComparisonCard(products) {
  if (!Array.isArray(products) || products.length < 2) return null;

  const productos = products.map((p) => {
    const consultar = isPrecioConsultar(p);
    return {
      id: p.id,
      nombre: p.nombre,
      marca: p.marca,
      imagen: resolveImageUrl(p),
      precio: consultar ? null : p.precio,
      precioFormateado: consultar ? null : formatPrice(p.precio),
      precioConsultar: consultar,
    };
  });

  const filas = TECH_FIELD_KEYS.map((key) => {
    const valoresCrudos = products.map((p) => {
      const spec = p.especificaciones || {};
      const value = spec[key];
      return value === null || value === undefined || value === '' ? null : value;
    });
    const hayAlMenosUnoConfirmado = valoresCrudos.some((v) => v !== null);
    if (!hayAlMenosUnoConfirmado) return null;
    return {
      key: key,
      label: TECH_FIELD_LABELS[key],
      valores: valoresCrudos.map((v) => (v === null ? 'No confirmado' : v)),
    };
  }).filter((fila) => fila !== null);

  return { tipo: 'comparison', productos: productos, filas: filas };
}
...
function compareProducts(ids) {
  ...
  return {
    ...
    comparison: buildComparisonCard(found),
  };
}
